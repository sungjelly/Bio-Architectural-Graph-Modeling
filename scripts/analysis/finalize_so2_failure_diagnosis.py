...
def complete_metadata():
    ...
    print(json.dumps({"report": str(FINAL / "report.md"), "result": str(RESULT), "baseline_runtime_seconds": read(BASE / "summary.json")["runtime_seconds"]}))

def repair_utc_metadata():
    # Narrow recovery for the initial metadata-validation failure, never a
    # rewrite of the immutable numerical or synthesis bundles.
    for root in (BASE, FINAL):
        manifest = read(root / "manifest.json")
        assert sha256_file(root / "manifest.json") == read(root / "_SUCCESS")["manifest_sha256"]
        for rel, info in manifest["files"].items(): assert sha256_file(root / rel) == info["sha256"]
    path = RESULT / "result.yaml"; record = yaml.safe_load(path.read_text())
    assert record["result_id"] == RESULT_ID
    for key in ("created_at_utc", "updated_at_utc"):
        value = record["provenance"][key]
        assert isinstance(value, str) and value.endswith("+00:00"), "Only the known unvalidated timestamp format may be repaired"
        record["provenance"][key] = value.replace("+00:00", "Z")
    path.write_text(yaml.safe_dump(record, sort_keys=False, width=110))
    complete_metadata()

if __name__ == "__main__":
    if sys.argv[1:] == ["--repair-utc-metadata"]:
        repair_utc_metadata()
    else:
