...
def main():
    ...
    if final.exists() or args.verify_only:
        ...
        return
    if (stage / "manifest.json").exists() and (stage / "_SUCCESS").exists():
        manifest = verify_report(stage)
        final.parent.mkdir(parents=True, exist_ok=True)
        require(not final.exists(), "Report destination already exists")
        os.rename(stage, final)
        verify_report(final)
        register_report(registry, final, manifest)
        print(json.dumps({"status": "published_verified_staging", "png": str(final / manifest["png"])}))
        return
    for snapshot in (stage / "source").rglob("*.py"):
        current_source = paths.project_root / snapshot.relative_to(stage / "source")
        require(current_source.is_file() and sha256_file(current_source) == sha256_file(snapshot),
                f"Source changed since this extraction began: {current_source.name}; use the saved source snapshot to resume")
    ...
    try:
        ...
        for source in sources:
            ...
            if destination.exists():
                require(sha256_file(source) == sha256_file(destination),
                        f"Source changed since this extraction began: {source.name}; use the saved source snapshot to resume")
            else:
        ...
        manifest = _receipt_with_self_hash({"schema": "so2_geometry_hl_report_v1", "status": "complete", "run_id": RUN_ID,
            ...
            "files": {key: value for key, value in _file_manifest(stage).items() if key != "_SUCCESS"}})
