#!/usr/bin/env python3
"""Materialize SO_1 radial graphs and shared relative-geometry caches."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build 14 exact SO1 radial-stratified graph caches. Matching "
            "CAN-01, CAN-09, and CAN-13 caches are hard-linked only after "
            "checksum, coordinate-order, and parameter verification."
        )
    )
    parser.add_argument(
        "--cohort-dir",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=paths.data_root / "processed/so1_14core_relative_qkv_graphs_v1",
    )
    parser.add_argument(
