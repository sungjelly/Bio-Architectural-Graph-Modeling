...
from spatial_benchmark.so1_training_observability import (
    AtomicLatestCheckpointStore,
    CHECKPOINT_SCHEMA,
    DurableEpochMetricsCSV,
    EPOCH_METRICS_COLUMNS,
    SO1_CORE_ALIASES,
    SO1ObservabilityError,
...
def _row(epoch: int, *, durations: tuple[float, ...] = ()) -> dict[str, object]:
    ...
    return build_epoch_metrics_row(
        ...
        per_core_masked_huber={
            ...
            for index, alias in enumerate(SO1_CORE_ALIASES)
        },
...
def test_epoch_row_contains_all_core_losses_and_operational_metrics() -> None:
    ...
    assert row["loss_so1_c01"] == pytest.approx(1.0)
    assert row["loss_so1_c14"] == pytest.approx(1.013)
...
def test_epoch_row_rejects_incomplete_ddp_epoch() -> None:
    ...
    with pytest.raises(SO1ObservabilityError, match="exactly 140"):
        build_epoch_metrics_row(
            run_id=str(values["run_id"]),
            model_seed=0,
            global_epoch=1,
            equal_core_mean_masked_huber=1.0,
            per_core_masked_huber={alias: 1.0 for alias in SO1_CORE_ALIASES},
...
def test_csv_resume_rejects_history_gap(tmp_path: Path) -> None:
    ...
    with pytest.raises(SO1ObservabilityError, match="end before"):
...
def test_failed_csv_replace_preserves_previous_complete_snapshot(
    ...
    monkeypatch.setattr(
        "spatial_benchmark.so1_training_observability.os.replace", fail_replace
    )
...
def test_strict_plateau_is_absolute_and_stops_only_when_eligible() -> None:
    first_eligible = strict_plateau_monitor_fields([1.0] * 150, model_seed=0)
    assert first_eligible["plateau_consecutive_passing_audits"] == 1
    assert first_eligible["plateau_should_stop"] is False
    ...
    assert flat["plateau_eligible_for_stopping"] is True
    assert flat["plateau_should_stop"] is True

    too_early = strict_plateau_monitor_fields([1.0] * 75, model_seed=0)
    assert too_early["plateau_conditions_passed"] is True
    assert too_early["plateau_eligible_for_stopping"] is False
    assert too_early["plateau_should_stop"] is False
...
def test_fixed_continuation_writes_only_final_epoch_300_checkpoint(
    ...
    with pytest.raises(SO1ObservabilityError, match="immutable|already exists"):
        store.save(_payload(2, model), completed_global_epochs=2)


def test_nonzero_rank_cannot_write_shared_outputs(tmp_path: Path) -> None:
    with pytest.raises(SO1ObservabilityError, match="rank zero"):
