"""Durable live metrics and latest-only checkpoints for SO2 training.
...
CORE_LOSS_COLUMNS = tuple(_loss_column(alias) for alias in SO2_CORE_ALIASES)
...
def _finite_float(value: object, *, field: str, nonnegative: bool = False) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float, np.number)):
        raise SO2ObservabilityError(f"{field} must be numeric.")
    result = float(value)
    if not math.isfinite(result) or (nonnegative and result < 0.0):
        qualifier = "finite non-negative" if nonnegative else "finite"
        raise SO2ObservabilityError(f"{field} must be {qualifier}.")
    return result
...
def plateau_monitor_fields(
    ...
    *,
    ...
    if values.ndim != 1 or values.size == 0 or not np.isfinite(values).all():
        raise SO2ObservabilityError("Plateau history must be finite and non-empty.")
...
def build_epoch_metrics_row(
    *,
    ...
    if set(per_core_masked_huber) != set(SO2_CORE_ALIASES):
        raise SO2ObservabilityError(
            "Per-core losses require exactly SO2-C15 through SO2-C28."
        )
    ...
    if complete_views != 140:
        raise SO2ObservabilityError(
            "A completed SO2 epoch must contain exactly 140 graph-mask views."
        )
    if masked_entries < 0:
        raise SO2ObservabilityError("Masked entry count cannot be negative.")
    if updates != 7 or cumulative_updates != epoch * 7:
        raise SO2ObservabilityError(
            "A completed SO2 epoch must add seven cumulative optimizer updates."
        )
    next_audit = (
...
class DurableEpochMetricsCSV:
    ...
    def __init__(self, run_root: str | Path, *, rank: int = 0) -> None:
        if int(rank) != 0:
            raise SO2ObservabilityError("Only distributed rank zero may write metrics.")
    ...
    def _read_rows(self) -> list[dict[str, str]]:
        ...
        if self.path.is_symlink() or not self.path.is_file():
            raise SO2ObservabilityError("Epoch metrics path must be a regular file.")
        with self.path.open("r", encoding="utf-8", newline="") as handle:
            reader = csv.DictReader(handle)
            if tuple(reader.fieldnames or ()) != EPOCH_METRICS_COLUMNS:
                raise SO2ObservabilityError("Epoch metrics CSV schema mismatch.")
            rows = list(reader)
        observed: list[int] = []
        for row in rows:
            try:
    ...
    def reconcile(self, *, checkpoint_epoch: int) -> int:
        ...
        if durable_epoch < 0:
            raise SO2ObservabilityError("Checkpoint epoch cannot be negative.")
        rows = self._read_rows()
    ...
    def append(self, row: Mapping[str, object]) -> bool:
        ...
        if set(row) != set(EPOCH_METRICS_COLUMNS):
            raise SO2ObservabilityError("Epoch metrics row has the wrong columns.")
        rows = self._read_rows()
...
class AtomicLatestCheckpointStore:
    ...
    def __init__(
        ...
        *,
        ...
        if self.completed_global_epochs < 0:
            raise SO2ObservabilityError("Completed checkpoint epoch cannot be negative.")
    ...
    def _validate_loaded(
        ...
        *,
        ...
        try:
            ...
            raise SO2ObservabilityError(f"Checkpoint is not reloadable: {path}.") from exc
        if not isinstance(value, Mapping):
            raise SO2ObservabilityError("Checkpoint payload must be a mapping.")
        payload = dict(value)
        if payload.get("checkpoint_schema") != CHECKPOINT_SCHEMA:
            raise SO2ObservabilityError("Checkpoint schema mismatch.")
        try:
        ...
        if epoch <= 0 or (expected_epoch is not None and epoch != expected_epoch):
            raise SO2ObservabilityError("Checkpoint completed epoch mismatch.")
        return payload
    ...
    def _write_bytes(
        ...
        *,
        ...
        if self.last_path.exists():
            raise SO2ObservabilityError("A finalized last checkpoint is immutable.")
        epoch_is_allowed = (
    ...
    def load_latest(self) -> dict[str, Any]:
        if not self.latest_path.is_file() or self.latest_path.is_symlink():
            raise SO2ObservabilityError("No regular latest checkpoint is available.")
    ...
    def finalize(
        ...
        *,
        ...
        if self.last_path.exists():
            raise SO2ObservabilityError("Final checkpoint already exists.")
        if payload is not None:
            if completed_global_epochs is None:
                raise SO2ObservabilityError("Final payload requires its epoch.")
        ...
        if not self.latest_path.is_file() or self.latest_path.is_symlink():
            raise SO2ObservabilityError("Cannot finalize without latest.ckpt.")
...
def checkpoint_payload_epoch(payload: Mapping[str, Any]) -> int:
    ...
    try:
        ...
        raise SO2ObservabilityError("Checkpoint lacks completed_global_epochs.") from exc
