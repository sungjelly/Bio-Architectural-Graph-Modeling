"""Identifier-safe preparation for the neutral SO_2 cores 15 through 28.

The cohort is selected only through the authoritative slide-qualified core map.
The one raw SO_2 field absent from that map (FOV 246) is audited and excluded
explicitly.  FOV and cell identifiers are temporary routing keys and are never
written to model arrays.  Coordinates are persisted separately for graph
construction and never enter ``node_covariates``.
"""
...
SO2_CORE_NUMBERS = tuple(range(15, 29))
SO2_ALIASES = tuple(f"SO2-C{number:02d}" for number in SO2_CORE_NUMBERS)
SO2_ALIAS_BY_NUMBER = dict(zip(SO2_CORE_NUMBERS, SO2_ALIASES, strict=True))
SO2_SOURCE_SLIDE = "SO_2"
UNMAPPED_EXCLUDED_FOV = 246
EXPECTED_UNMAPPED_FOV_CELLS = 340
EXPECTED_TOTAL_CELLS = 246_063
...
class SO2CohortContractError(ValueError):
    """Raised when SO_2 cohort routing or preprocessing fails closed."""
        ...
        return _canonical_sha256(
            {
                ...
                "slide": self.slide,
                "fovs": list(self.fovs),
                "explicitly_excluded_fov": UNMAPPED_EXCLUDED_FOV,
            }
        )
...
def resolve_so2_core_routes(core_map_csv: str | Path) -> tuple[SO2CoreRoute, ...]:
    """Resolve cores 15--28 from explicit ``(slide, fov)`` map keys.

    The SO_2 portion of the map must contain exactly the requested 14 core
    numbers.  FOV 246 must remain unmapped and can never enter a route.
    """

    frame = pd.read_csv(core_map_csv)
    required = {"slide", "core_label", "fov"}
    if not required.issubset(frame.columns):
        raise SO2CohortContractError(
            "Core map must contain slide, core_label, and fov columns."
        )
    frame = frame.loc[:, ["slide", "core_label", "fov"]].copy()
    if frame.isna().any().any():
        raise SO2CohortContractError("Core map routing fields cannot be missing.")
    try:
        ...
        raise SO2CohortContractError("Core map routing fields are invalid.") from exc
    if frame.duplicated(["slide", "fov"]).any():
        raise SO2CohortContractError(
            "Core map contains a duplicate slide-qualified FOV route."
        )

    slide_rows = frame.loc[frame["slide"] == SO2_SOURCE_SLIDE]
    requested_rows = frame.loc[frame["core_label"].isin(SO2_CORE_NUMBERS)]
    if len(requested_rows) != len(slide_rows) or set(
        requested_rows["slide"].tolist()
    ) != {SO2_SOURCE_SLIDE}:
        raise SO2CohortContractError(
            "Core labels 15 through 28 must resolve exclusively to SO_2."
        )
    if set(slide_rows["core_label"].tolist()) != set(SO2_CORE_NUMBERS):
        raise SO2CohortContractError(
            "The SO_2 map must contain exactly core labels 15 through 28."
        )
    if UNMAPPED_EXCLUDED_FOV in set(slide_rows["fov"].tolist()):
        raise SO2CohortContractError(
            "Raw SO_2 FOV246 must remain explicitly unmapped and excluded."
        )

    routes: list[SO2CoreRoute] = []
    for core_number in SO2_CORE_NUMBERS:
        selected = slide_rows.loc[slide_rows["core_label"] == core_number]
        if selected.empty:
            raise SO2CohortContractError(
                f"Core {core_number} must resolve to a nonempty SO_2 route."
            )
        fovs = tuple(sorted(int(value) for value in selected["fov"].unique()))
        routes.append(
            SO2CoreRoute(
                alias=SO2_ALIAS_BY_NUMBER[core_number],
                core_number=core_number,
                slide=SO2_SOURCE_SLIDE,
                fovs=fovs,
            )
        )
    qualified = [(route.slide, fov) for route in routes for fov in route.fovs]
    if len(qualified) != len(set(qualified)):
        raise SO2CohortContractError("SO_2 core routes overlap.")
    return tuple(routes)


def audit_so2_raw_fov_partition(
    metadata_csv: str | Path,
    routes: Sequence[SO2CoreRoute],
    *,
    chunksize: int = 100_000,
    expected_cell_counts_by_core: Mapping[int, int] | None = None,
    expected_total_cells: int = EXPECTED_TOTAL_CELLS,
    expected_excluded_fov_cells: int = EXPECTED_UNMAPPED_FOV_CELLS,
) -> dict[str, Any]:
    """Audit that mapped FOVs plus excluded FOV246 partition raw SO_2 cells."""

    if chunksize <= 0:
        raise SO2CohortContractError("Routing-audit chunk size must be positive.")
    expected_counts = dict(
        EXPECTED_CELL_COUNTS_BY_CORE
        if expected_cell_counts_by_core is None
        else expected_cell_counts_by_core
    )
    if set(expected_counts) != set(SO2_CORE_NUMBERS):
        raise SO2CohortContractError(
            "Expected cell counts must cover exactly cores 15 through 28."
        )
    ...
    try:
        ...
        raise SO2CohortContractError(
            "Could not audit raw SO_2 metadata FOV coverage."
        ) from exc

    selected_fovs = {fov for route in routes for fov in route.fovs}
    raw_fovs = set(counts_by_fov)
    unexpected = sorted(raw_fovs.difference(selected_fovs, {UNMAPPED_EXCLUDED_FOV}))
    missing = sorted(selected_fovs.difference(raw_fovs))
    if unexpected or missing:
        raise SO2CohortContractError(
            "Raw SO_2 FOVs are not partitioned by mapped cores plus FOV246 "
            f"(unexpected={unexpected}, missing={missing})."
        )
    excluded_cells = int(counts_by_fov.get(UNMAPPED_EXCLUDED_FOV, 0))
    if excluded_cells != int(expected_excluded_fov_cells):
        raise SO2CohortContractError(
            "Unmapped SO_2 FOV246 cell count changed: "
            f"expected {expected_excluded_fov_cells}, observed {excluded_cells}."
        )
    ...
    if observed_by_core != {
        ...
        raise SO2CohortContractError(
            "Mapped SO_2 per-core cell counts changed from the locked contract."
        )
    ...
    return {
        "source_slide": SO2_SOURCE_SLIDE,
        "mapped_core_numbers": list(SO2_CORE_NUMBERS),
        "mapped_fov_count": len(selected_fovs),
        "selected_cell_count": selected_cells,
        "per_core_cell_counts": {
            str(core): observed_by_core[core] for core in SO2_CORE_NUMBERS
        },
        "explicitly_excluded_unmapped_fov": UNMAPPED_EXCLUDED_FOV,
        "excluded_unmapped_fov_cell_count": excluded_cells,
        "raw_slide_cell_count": selected_cells + excluded_cells,
...
def _split_loaded_slide(
...
) -> list[tuple[SO2CoreRoute, CoreDataset]]:
    ...
    if observed_fovs != expected_fovs or UNMAPPED_EXCLUDED_FOV in observed_fovs:
        raise SO2CohortContractError(
            "Loaded SO_2 routing keys disagree with the mapped FOV partition."
        )
    result: list[tuple[SO2CoreRoute, CoreDataset]] = []
    for route in routes:
    ...
    if sum(core.n_cells for _, core in result) != loaded.n_cells:
        raise SO2CohortContractError("SO_2 core splitting lost or duplicated cells.")
    return result
...
def prepare_so2_14core_cohort(
    *,
    ...
    """Materialize the immutable, fit-only SO_2 core-15--28 cohort.

    All mapped FOVs are loaded together once and split by the authoritative
    routes.  This avoids rescanning the 1,000-gene SO_2 count table 14 times.
    """
    ...
    if destination.exists():
        raise FileExistsError(
            "SO_2 14-core output exists; immutable artifacts are not overwritten."
        )
    if chunksize <= 0 or not np.isfinite(pixel_size_um) or pixel_size_um <= 0:
        raise SO2CohortContractError("Chunk size and pixel size must be positive.")
    expected_counts = dict(
        EXPECTED_CELL_COUNTS_BY_CORE
        if expected_cell_counts_by_core is None
        else expected_cell_counts_by_core
    )
    routes = resolve_so2_core_routes(core_map_path)
    expression_path = discover_slide_raw_path(
        raw_root, SO2_SOURCE_SLIDE, "expression"
    )
    metadata_path = discover_slide_raw_path(raw_root, SO2_SOURCE_SLIDE, "metadata")
    routing_audit = audit_so2_raw_fov_partition(
        metadata_path,
        routes,
        chunksize=chunksize,
        expected_cell_counts_by_core=expected_counts,
        expected_total_cells=expected_total_cells,
        expected_excluded_fov_cells=expected_excluded_fov_cells,
    )
    ...
    try:
        loaded_slide = load_selected_core(
            ...
            CoreSelection(
                slide=SO2_SOURCE_SLIDE,
                fovs=all_fovs,
                label_policy="authoritative_so2_core_map_15_through_28_v1",
            ),
            chunksize=chunksize,
        ...
        raise SO2CohortContractError("SO_2 raw-count loading failed.") from exc
    ...
    if loaded_slide.n_genes != EXPECTED_N_GENES:
        raise SO2CohortContractError(
            "SO_2 cohort does not contain exactly 1,000 biological genes."
        )
    if any(name.startswith(CONTROL_PREFIXES) for name in gene_names) or len(
        set(gene_names)
    ) != EXPECTED_N_GENES:
        raise SO2CohortContractError(
            "SO_2 target schema contains controls or duplicate biological genes."
        )
    ...
    try:
        ...
        manifest: dict[str, Any] = {
            "artifact_kind": "so2_14core_pooled_full_core_preparation",
            "format_version": PREPARATION_SCHEMA_VERSION,
            "cohort": {
                "aliases": list(SO2_ALIASES),
                "original_core_numbers": list(SO2_CORE_NUMBERS),
                "source_slide": SO2_SOURCE_SLIDE,
            ...
            "assurances": {
                ...
                "unmapped_fov_246_excluded": True,
