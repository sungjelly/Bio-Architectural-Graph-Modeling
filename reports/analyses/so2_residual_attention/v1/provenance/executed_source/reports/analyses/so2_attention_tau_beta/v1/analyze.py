...
def summarize():
    ...
    tau_values = [r['tau'] for r in aggregates]
    panels=[('tau','Learned scale τ','Blues',min(1.8856180831641267,min(tau_values)),max(tau_values)),
    ...
    for ax,(key,title,cmap,vmin,vmax) in zip(axs,panels):
        ...
        for i in range(4):
            for j in range(8):
                rgb=im.cmap(im.norm(a[i,j]))[:3]
                brightness=sum(x*w for x,w in zip(rgb,(0.2126,0.7152,0.0722)))
                color='#161616' if brightness>0.52 else 'white'
    ...
    plt.close(fig)
    write_json(OUT/'figure_provenance.json',dict(render_script_sha256=sha256_file(Path(__file__)),
        per_head_csv_sha256=sha256_file(OUT/'per_head.csv'),
        summary_sha256=sha256_file(OUT/'summary.json'),png_sha256=sha256_file(OUT/'tau_beta.png'),
        pdf_sha256=sha256_file(OUT/'tau_beta.pdf')))
